/*export function iconAnimation() {
  const hero = document.querySelector('.box-hero__container');
  ...
  function getRandomPosition(isLeftSide) {
    ...
    const minTop = heroRect.height * 0.2;
    const maxTop = heroRect.height * 0.5;
    ...
    floatingImages.forEach((img, index) => {
      ...
      setTimeout(() => {
        ...
        img.style.transition = `
                        opacity ${fadeInDuration}ms cubic-bezier(0.4, 0, 0.6, 1)
                    `;
        img.style.opacity = '1';
        ...
        setTimeout(() => {
          // Резкое затухание
          img.style.transition = `
                            opacity ${fadeOutDuration}ms cubic-bezier(0.4, 0, 0.6, 1),
                        `;
          img.style.opacity = '0';
